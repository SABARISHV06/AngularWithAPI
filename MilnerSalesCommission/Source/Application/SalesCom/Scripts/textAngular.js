			...
			return {
				...
				link: function(scope, element, attrs, ngModel){
					...
					scope.displayElements = {
						...
						tapopover: angular.element('<div class="tapopover fade bottom" style="max-width: none; width: 305px;"></div>'),
						popoverArrow: angular.element('<div class="arrow"></div>'),
						popoverContainer: angular.element('<div class="tapopover-content"></div>'),
						resize: {
					...
					// Setup the tapopover
					scope.displayElements.tapopover.append(scope.displayElements.popoverArrow);
					scope.displayElements.tapopover.append(scope.displayElements.popoverContainer);
					scope.displayElements.scrollWindow.append(scope.displayElements.tapopover);

					scope.displayElements.tapopover.on('mousedown', function(e, eventData){
						/* istanbul ignore else: this is for catching the jqLite testing*/
						if(eventData) angular.extend(e, eventData);
						// this prevents focusout from firing on the editor when clicking anything in the tapopover
						e.preventDefault();
						return false;
					});

					// define the tapopover show and hide functions
					scope.showPopover = function(_el){
						scope.displayElements.tapopover.css('display', 'block');
						scope.reflowPopover(_el);
						$animate.addClass(scope.displayElements.tapopover, 'in');
					...
					scope.reflowPopover = function(_el){
						...
						if(scope.displayElements.text[0].offsetHeight - 51 > _el[0].offsetTop){
							scope.displayElements.tapopover.css('top', _el[0].offsetTop + _el[0].offsetHeight + 'px');
							scope.displayElements.tapopover.removeClass('top').addClass('bottom');
						}else{
							scope.displayElements.tapopover.css('top', _el[0].offsetTop - 54 + 'px');
							scope.displayElements.tapopover.removeClass('bottom').addClass('top');
						}
						var _maxLeft = scope.displayElements.text[0].offsetWidth - scope.displayElements.tapopover[0].offsetWidth;
						var _targetLeft = _el[0].offsetLeft + (_el[0].offsetWidth / 2.0) - (scope.displayElements.tapopover[0].offsetWidth / 2.0);
						scope.displayElements.tapopover.css('left', Math.max(0, Math.min(_maxLeft, _targetLeft)) + 'px');
						scope.displayElements.popoverArrow.css('margin-left', (Math.min(_targetLeft, (Math.max(0, _targetLeft - _maxLeft))) - 11) + 'px');
					};
					scope.hidePopover = function(){
						$animate.removeClass(scope.displayElements.tapopover, 'in', /* istanbul ignore next: dosen't test with mocked animate */ function(){
							scope.displayElements.tapopover.css('display', '');
							scope.displayElements.popoverContainer.attr('style', '');
							scope.displayElements.popoverContainer.attr('class', 'tapopover-content');
						});
					};
