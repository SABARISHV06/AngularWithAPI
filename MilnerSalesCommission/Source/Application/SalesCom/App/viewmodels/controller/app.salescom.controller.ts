...
module mtisalescom {
    ...
    export interface ISalesCommissionScope extends ng.IScope {
        ...
        UpdateEditNumberOfDays(index: number, cval:any): void;
      ...
      //  active: any;
        ...
        RemoveEditNewPayrollConfigRow(index: number, row: Payroll): void;
        Payrollconfigrow: Payroll[];
        NewPayrollconfigrow: Payroll[];
        TempPayroll: Payroll;
        PlusPayroll: Payroll;
        buildDateTo: any;
		DaysArr: any[];
    ...
    class SalesCommissionControl {
        ...
        constructor(private $location: ng.ILocationService, $rootScope, public $scope: ISalesCommissionScope, private $q: ng.IQService, public $http: ng.IHttpService, $routeParams,
            ...
            $scope.UpdateEditNumberOfDays = (index, cval): void => {
                var isLeapYear = function () {
                    var year = $scope.TempPayroll.Year || 0;
                    return ((year % 400 === 0 || year % 100 !== 0) && (year % 4 === 0)) ? 1 : 0;
                }
           ...
           $scope.GoToEditPayrollConfiguration = (year): void => {
               ...
               this.SalesComService.GetPayrollForEdit(year).then((data: ng.IHttpPromiseCallbackArg<Payroll[]>): void => {
                ...
                $scope.NewPayrollconfigrow = new Array<Payroll>();
           }
           ...
           $scope.UpdatePayroll = (): void => {
               ...
               this.SalesComService.UpdatePayroll($scope.NewPayrollconfigrow).then((data: ng.IHttpPromiseCallbackArg<Boolean>): void => {
                   if (<Boolean>data == true) {
           ...
           $scope.ModalDeactivatedBranch = (branchid, index): void => {
               ...
               this.SalesComService.GetDeActivateBranch(branchid).then((data: ng.IHttpPromiseCallbackArg<boolean>): void => {
                   if (<Boolean>data == true) {
                       $scope.ShowAlertDialog("Location Deactivate Alert", "This Location has associated Commission Slips for the current accounting period / it is mapped as a Primary Location to Employees. You can not deactivate this Location.", function () {});
                   }
                   else {
